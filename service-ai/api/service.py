...
def _chat_sync(
    ...
    system = (
        "你是知识库问答助手。用户每一条消息中会给出【当前问题】与【检索到的参考片段】（编号 [1]、[2]…）。"
        "若片段与问题相关，请优先依据片段作答，可归纳、转述，不编造片段中不存在的事实。"
        "若片段与问题无关或完全无法支撑任何要点，直接回答问题即可。"
        "每次你要回答用户最新的消息中的当前问题，同时你也可以参考历史消息，包括User和Assistant的。"
    )
